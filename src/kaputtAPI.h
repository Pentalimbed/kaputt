...
public:
    ...
    virtual bool         is_ready()                                                       = 0; // check if everything's loaded
    ...
    virtual bool         submit(
                ...
                std::set<std::string, std::less<>>* banned_tags   = nullptr) = 0; // request playing one of the registered animations with extra tag requirements.
};
